...
class TestPathfinding(TestCase):
    def test_pathfind_with_obstacle_clipping(self):
        ...
        pathfinding = Pathfinding(targets=list_balls, start=start,
                                  obstacle=Rectangle(Coordinate(20, 20), Coordinate(30, 30)))

        ball = pathfinding.get_closest(start)
        self.assertEqual(list_balls[0].center.x, ball.center.x)
        list_balls.__delitem__(0)
        self.assertEqual(len(list_balls), 2)
        ball = pathfinding.get_closest(start)
        self.assertEqual(list_balls[0].center, ball.center)
        list_balls.__delitem__(0)
        ball = pathfinding.get_closest(start)
    ...
    def test_pathfind(self):
        ...
        ball = pathfinding.get_closest(start)
        self.assertEqual(ball.center, list_balls[2].center)
